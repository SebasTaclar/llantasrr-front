<template>
  <div class="apple-watch-page">
    <!-- Hero Banner Section con Carrusel -->
    <section class="hero-banner">
      <div class="carousel-container">
        <!-- Slides del carrusel dinámicos desde productos de showcase de Apple Watch -->
        <div class="carousel-track" :style="{ transform: `translateX(-${currentSlide * 100}%)` }">
          <div
            v-for="product in watchShowcaseProducts"
            :key="product.id"
        ...
        <div class="carousel-indicators">
          <button
            v-for="(product, index) in watchShowcaseProducts"
            :key="product.id"
            class="indicator"
    ...
    <section class="products-section">
      <div class="container">
        ...
        <div v-else-if="watchProducts.length > 0" class="products-container">
          <div
        ...
        <div v-else class="empty-state">
          <p>No hay Apple Watchs disponibles en este momento</p>
        </div>
      </div>
    </section>
    ...
    <section class="info-section">
      <div class="container">
        <div class="info-content-centered">
          <h2 class="info-title">¿Por qué elegir Apple Watch?</h2>
          <p class="info-text">
            Tu compañero perfecto para un estilo de vida saludable y conectado. Apple Watch monitorea
            tu salud 24/7 con ECG, oxígeno en sangre y detección de caídas. Mantente activo con
            entrenamientos avanzados, alcanza tus objetivos de fitness y recibe notificaciones
            importantes directamente en tu muñeca. Resistente al agua, batería de todo el día y
            perfectamente integrado con tu ecosistema Apple.
          </p>
        </div>
      </div>
    </section>
        ...
        <div class="features-grid">
          <div class="feature-card">
            ...
            <p class="feature-text">
              Una nueva forma de interactuar con tu Apple Watch. Notificaciones en vivo,
...
// Carrusel dinámico basado en productos de showcase de Apple Watch
const currentSlide = ref(0)
let autoplayInterval: number | null = null

// Obtener la categoría de Apple Watch
const watchCategory = computed(() => {
  return categories.value.find(cat =>
    cat.name.toLowerCase().includes('watch') ||
    cat.name.toLowerCase().includes('reloj')
  )
})

// Filtrar productos showcase de Apple Watch para el carrusel
const watchShowcaseProducts = computed(() => {
  if (!watchCategory.value) return []

  return showcaseProducts.value.filter(p =>
    p.category === watchCategory.value!.id ||
    p.category.toLowerCase().includes('watch')
  )
})

const nextSlide = () => {
  const total = watchShowcaseProducts.value.length || 1
...
// Filtrar productos de Apple Watch
const watchProducts = computed(() => {
  if (!watchCategory.value) return []

  return regularProducts.value
    .filter(p => {
      const categoryMatch = p.category === watchCategory.value!.id
